...
describe XPath::HTML do
  ...
  describe '#link' do
    ...
    it("finds links by content without caring about whitespace") { get('My whitespaced link').should == 'link-whitespace' }
    ...
    it("finds links by an image's alt attribute") { get('Alt link').should == 'link-img' }
    it("finds links by an image's approximate alt attribute") { get('Alt').should == 'link-img' }
  ...
  describe '#button' do
    ...
    context "with submit type" do
      ...
      it("finds prefers buttons with exact value ") {   all('exact value submit').should == ['exact-value-submit', 'not-exact-value-submit'] }
    end
    ...
    context "with button type" do
      ...
      it("finds prefers buttons with exact value ") { all('exact value button').should == ['exact-value-button', 'not-exact-value-button'] }
    end
    ...
    context "with button tag" do
      ...
      it("finds buttons by the text of their child elements") { get('emphatic').should == 'btag-with-children' }
